...
# Specific for 16S rRNA-based amplicons (SILVA 138 db)
...
silva.fasta_path <- as.character(args[4])
silvaspec.fasta_path <- as.character(args[5])
setwd(current_path)

# Importing data from previous script and select ASVs according to Metaxa2 output and replace ASV sequences with targeted extracted sequences:
cat("\n", "==================== Importing data... ====================", "\n", "\n")
...
# Assigning taxonomy:
cat("\n", "==================== Assigning taxonomy using RDP classifier against SILVA 138... ====================", "\n", "\n")
taxa <- assignTaxonomy(seqtab_filt, silva.fasta_path, minBoot=80, multithread=TRUE, tryRC=TRUE, verbose=TRUE)
taxa <- addSpecies(taxa, silvaspec.fasta_path)
...
taxonomy <- as.data.frame(taxa) %>% 
				transmute(taxonomy = paste(paste0("k__", Kingdom), paste0("p__", Phylum), paste0("c__", Class), paste0("o__", Order), paste0("f__", Family), paste0("g__", Genus), paste0("s__", Genus, " ", Species), sep = ';'))
...
# Calculating read counts and saving the filtered FASTA and ASV table for later...
